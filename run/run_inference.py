...
if __name__=="__main__":
    ...
    use_seperate = True
    stable_diffusion_repo_path = "/data/local_userdata/zhujiajun/Marigold/checkpoint/Marigold_v1_merged_2"
    ...
    parser.add_argument(
        ...
        type=str,
        default='/data/local_userdata/zhujiajun/Marigold/checkpoint/Marigold_v1_merged_2',
    ...
    parser.add_argument(
        "--output_dir", type=str, default='/data/local_userdata/zhujiajun/Marigold/outputs321', help="Output directory."
    )
    ...
    video_dir = "/data/dataset/kitti/raw_data/2011_09_26/2011_09_26_drive_0013_sync/image_02/data/"
    frames = glob.glob(video_dir+'/*.png')
    frames.sort(key=lambda x: int(x[-14:-4]))
    for i in range(len(frames)):
        with torch.no_grad():
            os.makedirs(output_dir, exist_ok=True)
            # load the example image.
            input_image_pil = Image.open(frames[i])
            ...
            colored_save_path = os.path.join(
                output_dir_color, "{}.png".format(str(i).zfill(10))
            )
