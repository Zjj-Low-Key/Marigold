...
def log_validation(vae,text_encoder,tokenizer,unet,args,accelerator,weight_dtype,scheduler,epoch,
                   input_image_path="/data/dataset/VKITTI/vkitti_2.0.3/Scene06/clone/frames/rgb/Camera_0/rgb_00269.jpg"
                   ):
    ...
    try:
        ...
        logger.info('enable_xformers_memory_efficient_attention failed')
        pass  
    ...
    with torch.no_grad():
        ...
        plt.imsave(colored_save_path,depth_colored)
...
def main():
    ...
    with accelerator.main_process_first():
        ...
        print('args.dataset_path, train_list and val_list hard-coded, not set')
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_loader):
            with accelerator.accumulate(unet):
                ...
                disparity_data = batch['depth']

                # print(left_image_data.shape)
                # print(f'img | min:{left_image_data.min().data}, max:{left_image_data.max().data}, mean:{left_image_data.mean().data}, std:{left_image_data.std().data}')
              
                # print(disparity_data.shape)
                # print(f'min:{disparity_data.min().data}, max:{disparity_data.max().data}, mean:{disparity_data.mean().data}, std:{disparity_data.std().data}')
                ...
                disparity_data_resized = resize_max_res_tensor(disparity_data,is_disp=False) #range in (0-1)
                
                left_image_data_resized = (left_image_data_resized-0.5) *2.0
                # right_image_data_resized = (right_image_data_resized-0.5)*2.0
                disparity_data_resized = (disparity_data_resized-0.5)*2.0
                ...
                h_disp = vae.encoder(disparity_data_resized.to(weight_dtype))
