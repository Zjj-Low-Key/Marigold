...
def main():
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        ...
        unet = UNet2DConditionModel.from_config(args.pretrained_model_name_or_path, subfolder="unet", in_channels=8, sample_size=96,low_cpu_mem_usage=False,ignore_mismatched_sizes=True)
        state_dict = torch.load("/home/xiangmochu/Accelerator-Simple-Template-depth_est_diff/checkpoint/Marigold_v1_merged_2/unet/diffusion_pytorch_model.bin", map_location="cpu")
        the_layer_weight = state_dict['conv_in.weight']
        state_dict.update({'conv_in.weight': torch.cat([the_layer_weight/2, the_layer_weight/2],dim=1)})
